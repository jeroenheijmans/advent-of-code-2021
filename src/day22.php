...
class Cube {
  ...
  public function cutAgainst($other) {
    if (!$this->overlapsWith($other)) throw new Error("Cannot cut if no overlap exists");

    if ($other->minx > $this->minx && $other->minx <= $this->maxx) {
      return new Collection([
        new Cube($this->minx, $this->miny, $this->minz, $other->minx-1, $this->maxy, $this->maxz),
        new Cube($other->minx, $this->miny, $this->minz, $this->maxx, $this->maxy, $this->maxz),
      ]);
    }
    if ($other->maxx >= $this->minx && $other->maxx < $this->maxx) {
      return new Collection([
        new Cube($this->minx, $this->miny, $this->minz, $other->maxx, $this->maxy, $this->maxz),
        new Cube($other->maxx+1, $this->miny, $this->minz, $this->maxx, $this->maxy, $this->maxz),
      ]);
    }
    
    if ($other->miny > $this->miny && $other->miny <= $this->maxy) {
      return new Collection([
        new Cube($this->minx, $this->miny, $this->minz, $this->maxx, $other->miny-1, $this->maxz),
        new Cube($this->minx, $other->miny, $this->minz, $this->maxx, $this->maxy, $this->maxz),
      ]);
    }
    if ($other->maxy >= $this->miny && $other->maxy < $this->maxy) {
      return new Collection([
        new Cube($this->minx, $this->miny, $this->minz, $this->maxx, $other->maxy, $this->maxz),
        new Cube($this->minx, $other->maxy+1, $this->minz, $this->maxx, $this->maxy, $this->maxz),
      ]);
    }
    
    if ($other->minz > $this->minz && $other->minz <= $this->maxz) {
      return new Collection([
        new Cube($this->minx, $this->miny, $this->minz, $this->maxx, $this->maxy, $other->minz-1),
        new Cube($this->minx, $this->miny, $other->minz, $this->maxx, $this->maxy, $this->maxz),
      ]);
    }
    if ($other->maxz >= $this->minz && $other->maxz < $this->maxz) {
      return new Collection([
        new Cube($this->minx, $this->miny, $this->minz, $this->maxx, $this->maxy, $other->maxz),
        new Cube($this->minx, $this->miny, $other->maxz+1, $this->maxx, $this->maxy, $this->maxz),
      ]);
    }
  }
  ...
  foreach ($data as $line) {
    ...
    do {
      ...
      foreach ($overlappingCubes as $other) {
        $result = $other->cutAgainst($current);
        $newstate = $newstate->concat($result);
      }

      $state = $newstate;

      if ($loop++ > 100) throw new Error("bug?");
    } while (true);
