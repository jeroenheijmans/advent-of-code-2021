  ...
  foreach ($data as $row) {
    ...
    while ($loopstop++ < 1) {
      foreach ($signals as $signal) {
        ...
        if (count($signal) === 6) {
          // Look for the 6, which is the only 6-segment digit that
          // has not have complete overlap with the 7 (as 0 and 9 do)
          $diff = array_diff($theSeven, $signal);
          if (count($diff) === 1) {
            $mappings["c"] = $diff;
          }
        }
        ...
        foreach ($mappings as $key => $value) {
          // mapping found? remove it from other options
          if (count($value) === 1) {
    ...
    $output = "";
    foreach ($row[1] as $part) {
      $signal = str_split($part);
      if (count($signal) === 2) $output .= "1";
      if (count($signal) === 3) $output .= "7";
      if (count($signal) === 4) $output .= "4";
      if (count($signal) === 7) $output .= "8";

      if (count($signal) === 5) {
        if (in_array(current($mappings["e"]), $signal)) $output .= "2";
        else if (in_array(current($mappings["b"]), $signal)) $output .= "5";
        else$output .= "3";
      }

      if (count($signal) === 6) {
        if (!in_array(current($mappings["d"]), $signal)) $output .= "0";
        if (!in_array(current($mappings["c"]), $signal)) $output .= "6";
        if (!in_array(current($mappings["e"]), $signal)) $output .= "9";
      }
    }
    // echo "OUTPUT = $output\n";
    $answer += intval($output);
  }
